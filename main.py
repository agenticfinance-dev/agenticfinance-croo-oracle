...
async def send_telegram_message(chat_id, text, reply_markup=None):
    # Truncate messages to avoid Telegram limits
    if len(text) > 4000:
        text = text[:3900] + "...\n\n(truncated)"
...
# ==================== SIGNAL FORMATTING (DRY) ====================
def format_signal(signal):
    """Centralized signal formatting for all outputs."""
...
async def get_ohlcv(asset, interval="1h", limit=100):
    ...
    needed = min(limit, 50)  # settle for a partial fetch rather than nothing, but never below the model's minimum
...
# ==================== PAPER TRADE MONITOR ====================
_notified_milestones = set()  # tracks (trade_id, milestone) pairs already announced, so we don't re-spam every 30s
...
async def paper_trade_monitor():
    ...
    while not shutdown_event.is_set():
        ...
        try:
            ...
            # forget milestones for trades that are no longer open (closed/gone), to bound memory growth
            ...
            for row in rows:
                ...
                if direction == "LONG":
                    if current >= tp3:
                        ...
                        update_paper_trade_sl(trade_id, entry)  # actually move the stop to breakeven
                        await send_telegram_message(
                            user_id,
...
async def correlation_matrix(symbols):
    """Compute real correlation matrix from OHLCV data."""
    prices = {}
    for symbol in symbols:
        klines, _ = await get_ohlcv(symbol, "1h")
        if klines and len(klines) > 50:
            closes = [float(k[4]) for k in klines]
            prices[symbol.replace("USDT", "")] = closes[-50:]  # last 50 closes
...
async def correlation():
    await ensure_scan()
    # Use first 5 assets for demo
    assets = [a for a in ASSETS[:5] if get_current_price(a) > 0]
    if len(assets) < 2:
        return api_response(True, {"message": "Insufficient data for correlation"})

    result = await correlation_matrix(assets)
    if result is None:
        return api_response(True, {"message": "Could not compute correlation"})
    return api_response(True, {"matrix": result, "note": "Real correlation from 1h OHLCV data"})
...
async def handle_message(chat_id, text, user_id):
    ...
    if text == "/start":
        ...
        demo_text += f"Version: {demo_data.get('version')}\n"
